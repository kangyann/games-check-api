...
export default function AdminClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="font-[family-name:var(--font-geist-sans)] flex min-h-screen bg-surface text-foreground">
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[260px] bg-white border-r border-border flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex items-center gap-2.5 px-5 py-4 border-b border-border">
          <Image src={LogoIcon} alt="Mylix" width={28} height={28} />
          <div>
            <h1 className="text-sm font-bold text-foreground leading-none">Mylix Admin</h1>
          ...
          {sidebarLinks.map((link) => {
            ...
            return (
              <Link
                ...
                href={link.href}
                onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition ${isActive ? "bg-primary-subtle text-primary font-semibold" : "text-muted hover:bg-surface-hover hover:text-foreground"}`}
              >
                <link.icon size={16} />
                {link.label}
              </Link>
