...
const sidebarLinks = [
  ...
  { href: "/dashboard/api-keys", label: "API Keys", icon: FaKey },
  { href: "/dashboard/usage", label: "Usage", icon: FaChartLine },
    ...
    <div className="font-[family-name:var(--font-geist-sans)] flex min-h-screen bg-surface text-foreground">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-[260px] bg-white border-r border-border flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="flex items-center gap-2.5 px-5 py-4 border-b border-border">
          ...
          <div>
            <h1 className="text-sm font-bold text-foreground leading-none">Mylix App</h1>
            <span className="text-[10px] text-muted">Dashboard</span>
          </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {sidebarLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition ${isActive ? "bg-primary-subtle text-primary font-semibold" : "text-muted hover:bg-surface-hover hover:text-foreground"}`}>
                <link.icon size={16} />{link.label}
              </Link>
            );
          })}
          <div className="pt-3 mt-3 border-t border-border-light">
            <Link href="/docs" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-muted hover:bg-surface-hover hover:text-foreground transition"><FaBook size={16} />Documentation</Link>
          </div>
        </nav>
        <div className="p-3 border-t border-border">
          <div className="flex items-center gap-2.5 px-3 py-2">
            <div className="w-8 h-8 rounded-full bg-primary-light flex items-center justify-center text-primary text-xs font-bold">{user?.username?.[0]?.toUpperCase() || "U"}</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-sm border-b border-border px-6 py-3 flex items-center gap-3">
          ...
          <h2 className="text-sm font-semibold text-foreground">{sidebarLinks.find((l) => l.href === pathname)?.label || "Dashboard"}</h2>
        </header>
